...
function camelCaseToKebab(camelCaseString) {
...
void (function () {
    ...
    const mapContainer = pointsPopup?.querySelector('.popup__inner')
    if (!pointsPopup || !mapContainer) return
    let callPopupElement = null
    const onSetPointButtonClick = (e) => {
    ...
    pointsPopup.addEventListener('opened', async (customEvent) => {
        callPopupElement = customEvent.detail.trigger
        const dataPoints = Array.from(document.querySelectorAll('.points-popup__data-list div')).map((i) => i.dataset)
...
function createMarkerBalloon(pointInfo, onButtonClick) {
    ...
    const setPointButton = balloon.querySelector('.points-popup__balloon-button')
    setPointButton?.addEventListener('click', onButtonClick)
    return balloon
}
function initPointsMap(options) {
...
function setPointList(props) {
    const { dataPoints, onSetPointButtonClick } = props
    const pointListElement = document.querySelector('.points-popup__list')
    ...
    const layout = document.querySelector('.points-popup__item._layout')
    if (!layout) {
        console.log('Вероятно, кто-то удалил HTML разметку из верстки')
        return
    }
    dataPoints.forEach((point) => {
        const pointElement = layout.cloneNode(true)
        ...
        const setPointButton = pointElement.querySelector('.points-popup__item__button')
