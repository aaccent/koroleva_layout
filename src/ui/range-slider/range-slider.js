...
class RangeSlider {
    ...
    numberWithFallback(number, fallback) {
        return typeof number === 'number' ? number : fallback
    }
    /** Если `value` будет меньше или больше лимитов, то возвращает лимит, иначе `value` */
    boundsValue(value) {
    ...
    initTrack() {
        const track = this.el.querySelector('.range-slider__track')
    ...
    initThumb(key) {
        const thumb = this.el.querySelector(`.range-slider__thumb--${key}`)
    ...
    initLine() {
        const line = this.el.querySelector('.range-slider__line')
        if (!line) throw new Error('У .range-slider нет .range-slider__line')
        this.line = line
        this.setLine()
    }
    initInput(key) {
        const input = this.el.querySelector(`.range-slider__input--${key}`)
        ...
        input.addEventListener('input', (event) => this.inputHandler(event, key))
    ...
    inputHandler(event, key) {
        let value = parseInt(event.currentTarget.value.replaceAll(/\D/g, ''))
...
document.querySelectorAll('.range-slider').forEach((range) => new RangeSlider(range))
